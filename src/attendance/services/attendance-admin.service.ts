...
export class AttendanceAdminService {
  ...
  async assignShift(
    ...
    if (!employee) {
      throw new NotFoundException(
        'Employee not found in this company workspace',
      );
    }
  ...
  async manualOverride(
    ...
    try {
      ...
      if (
        dto.attendanceStatus === AttendanceStatus.ON_LEAVE ||
        record.attendanceStatus === AttendanceStatus.ON_LEAVE
      ) {
        if (
          dto.attendanceStatus !== undefined &&
          dto.attendanceStatus !== record.attendanceStatus
        ) {
          throw new BadRequestException(
            'Cannot override ON_LEAVE status directly. Use the manual attendance API to ensure leave balances are updated correctly.',
          );
        }
      }
      ...
      if (finalCheckIn && finalCheckOut) {
        ...
        updateData.totalHours = Math.max(
          ...
          parseFloat(
            (
              (totalDurationMs - breakMinutes * 60 * 1000) /
              (1000 * 60 * 60)
            ).toFixed(2),
          ),
        );
  ...
  async manualAttendance(
    ...
    if (dto.status === AttendanceStatus.ON_LEAVE && !dto.leaveTypeId) {
      throw new BadRequestException(
        'leaveTypeId is mandatory when marking attendance as LEAVE',
      );
    }
    ...
    try {
      ...
      if (
        previousRecord &&
        previousRecord.attendanceStatus === AttendanceStatus.ON_LEAVE
      ) {
        if (
          ...
          const logs = await this.logModel.findAll({
            where: {
              ...
              actionType: {
                [Op.in]: [
                  AttendanceActionType.ADMIN_MARKED,
                  AttendanceActionType.AUTO_CORRECTION,
                ],
              },
            },
            order: [['id', 'DESC']],
      ...
      const finalCheckIn = dto.checkInTime ? new Date(dto.checkInTime) : null;
      ...
      if (finalCheckIn && finalCheckOut) {
        ...
        totalHours = Math.max(
          0,
          parseFloat((totalDurationMs / (1000 * 60 * 60)).toFixed(2)),
        );
      }
      ...
      await this.logModel.create(
        {
          ...
          metadata: {
            ...
            leaveTypeId:
              dto.status === AttendanceStatus.ON_LEAVE ? dto.leaveTypeId : null,
          },
        },
        { transaction: t },
      );
