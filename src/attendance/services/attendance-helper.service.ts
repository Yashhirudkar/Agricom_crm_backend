...
export class AttendanceHelperService {
  ...
  public getLocalTimeDetails(timezone = 'Asia/Kolkata', inputDate = new Date()): { todayDateStr: string, minutesOfDay: number, jsDay: number } {
  ...
  public calculateLateMinutes(minutesOfDay: number, shiftStartTime: string, gracePeriod: number): number {
  ...
  public async getActiveEmployee(employeeId: number, companyId: number): Promise<Employee> {
