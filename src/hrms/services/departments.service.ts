...
export class DepartmentsService {
  ...
  async createDepartment(
    ...
    if (data.departmentCode) {
      const existing = await this.departmentModel.findOne({
        where: { companyId, departmentCode: data.departmentCode },
      });
      if (existing) {
        throw new ConflictException(
          `Department code '${data.departmentCode}' already exists in this company`,
        );
      }
    }
    ...
    if (data.parentDepartmentId) {
      const parent = await this.departmentModel.findOne({
        where: { id: data.parentDepartmentId, companyId },
      });
      if (!parent) throw new NotFoundException('Parent department not found');
    }

    if (data.departmentHeadId) {
      const head = await this.employeeModel.findOne({
        where: { id: data.departmentHeadId, companyId },
      });
  ...
  private async isCircularDepartment(
    departmentId: number,
    newParentId: number,
    companyId: number,
  ): Promise<boolean> {
  ...
  async updateDepartment(
    ...
    if (
      ...
      if (!parent) throw new NotFoundException('Parent department not found');
      const isCircular = await this.isCircularDepartment(
        department.id,
        data.parentDepartmentId,
        companyId,
      );
    ...
    if (
      ...
      data.departmentHeadId !== department.departmentHeadId
    ) {
      const head = await this.employeeModel.findOne({
        where: { id: data.departmentHeadId, companyId },
      });
  ...
  async deleteDepartment(
    ...
    const designationCount = await this.designationModel.count({
      where: { departmentId: id },
    });
    if (designationCount > 0) {
      throw new ConflictException(
        `Cannot delete department because it is linked to ${designationCount} designation(s).`,
      );
    }
    ...
    if (employeeCount > 0) {
      throw new ConflictException(
        `Cannot delete department because it is linked to ${employeeCount} employee(s).`,
      );
    }
  ...
  async getDepartmentsForOptions(
    companyId: number,
    search?: string,
    page: string = '1',
    limit: string = '10',
