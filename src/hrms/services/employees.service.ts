...
export class EmployeesService {
  ...
  private async generateEmployeeCode(companyId: number): Promise<string> {
    ...
    if (
      ...
      lastEmployee.employeeCode.startsWith('EMP')
    ) {
  ...
  async createEmployee(
    ...
    if (data.designationId) {
      const desig = await this.designationModel.findOne({
        where: { id: data.designationId, companyId },
      });
    ...
    if (data.employeeCode) {
      const existing = await this.employeeModel.findOne({
        where: { companyId, employeeCode: data.employeeCode },
      });
      if (existing) {
        throw new BadRequestException(
          `Employee code '${data.employeeCode}' already exists in this company`,
        );
      }
    }

    const employeeCode =
      data.employeeCode || (await this.generateEmployeeCode(companyId));
  ...
  async transitionLifecycle(
    ...
    const newStatus = targetStatus as EmployeeStatus;

    // Workflow validation rules
    if (
      newStatus === EmployeeStatus.CONFIRMED &&
      currentStatus === EmployeeStatus.TERMINATED
    ) {
      throw new BadRequestException(
        'A terminated employee cannot be confirmed',
      );
    }
    if (
      newStatus === EmployeeStatus.ACTIVE &&
      currentStatus === EmployeeStatus.RESIGNED
    ) {
      throw new BadRequestException(
        'A resigned employee cannot return to active without proper re-onboarding',
      );
    }
    if (newStatus === EmployeeStatus.RESIGNED) {
      if (
        currentStatus !== EmployeeStatus.CONFIRMED &&
        currentStatus !== EmployeeStatus.NOTICE_PERIOD
      ) {
        throw new BadRequestException(
          'Only confirmed or notice period employees can resign',
        );
      }
    }
    ...
    if (currentStatus === newStatus) {
      throw new BadRequestException(
        `Employee is already in ${newStatus} state`,
      );
    }
    ...
    try {
      ...
      return {
        message: `Employee successfully transitioned to ${newStatus}`,
        employee: updated,
      };
    } catch (err) {
      await t.rollback();
  ...
  async updateEmployee(
    ...
    if (data.managerId !== undefined && data.managerId !== employee.managerId) {
      ...
      if (data.managerId !== null) {
        ...
        const isCircular = await this.orgService.isCircularManager(
          employee.id,
          data.managerId,
          companyId,
        );
        if (isCircular) {
          throw new BadRequestException(
            'Circular reporting hierarchy detected (e.g. manager reports back to this employee)',
          );
        }
      }
    }
    ...
    try {
      ...
      if (data.createLogin && data.password && !employee.userId) {
        ...
        if (data.roleId !== undefined) {
          await this.usersService.updateUserCompanyRole(
            updatedUserId,
            companyId,
            data.roleId,
            t,
    ...
    const { rows, count } = await this.employeeModel.findAndCountAll({
      ...
      include: [
        ...
        },
        {
          ...
          attributes: ['id', 'oldStatus', 'newStatus', 'remarks', 'createdAt'],
        },
      ],
    });
    ...
    const employee = await this.employeeModel.findOne({
      ...
      include: [
        ...
        },
        {
          ...
          include: [
            { model: UserCompany, where: { companyId }, required: false },
          ],
        },
        {
          model: EmployeeLifecycleLog,
