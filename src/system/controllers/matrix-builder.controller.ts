...
export class MatrixBuilderController {
  ...
  @Post('module')
  @RequirePermission('system:manage_matrix')
  @AuditLog({ entityType: 'AppModule' })
  async createModule(
    @Req() req: any,
    @Body() dto: { name: string; icon_name?: string; sort_order?: number },
  ) {
  ...
  @Post('resource')
  @RequirePermission('system:manage_matrix')
  @AuditLog({ entityType: 'ModuleResource' })
  ...
  @Post('action')
  @RequirePermission('system:manage_matrix')
  @AuditLog({ entityType: 'ResourceAction' })
  async createAction(
    @Req() req: any,
    @Body()
    dto: {
  ...
  async deleteModule(@Req() req: any, @Param('id') id: string) {
    return this.matrixBuilderService.deleteModule(
      req.user.id,
      parseInt(id, 10),
    );
  }
  ...
  async deleteResource(@Req() req: any, @Param('id') id: string) {
    return this.matrixBuilderService.deleteResource(
      req.user.id,
      parseInt(id, 10),
    );
  }
  ...
  async deleteAction(@Req() req: any, @Param('id') id: string) {
    return this.matrixBuilderService.deleteAction(
      req.user.id,
      parseInt(id, 10),
    );
  }
