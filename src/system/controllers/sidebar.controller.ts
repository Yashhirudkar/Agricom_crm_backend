...
export class SidebarController {
  ...
  @Patch('item/:id/move')
  @RequirePermission('system:manage_sidebar')
  async moveItem(@Req() req: any, @Param('id') id: string, @Body() dto: { folder_id: number | null }) {
    return this.sidebarService.moveItem(req.user.id, parseInt(id, 10), dto.folder_id);
  }

  @Patch('reorder')
  @RequirePermission('system:manage_sidebar')
  async reorder(@Req() req: any, @Body() body: { updates: { id: number; type: 'FOLDER' | 'ITEM'; sort_order: number }[] }) {
